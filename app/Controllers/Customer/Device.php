...
class Device extends BaseController
{
   ...
   public function __construct(){
    ...
    public function report($device_id=""){
        ...
        $tdata ['ajax_url_for_data']=base_url("Customer/Device/report_ajax/".$device_id);
...
//            ads_media_master.media_url,
           ...
           $db =db_connect();
            ...
            $device_reports->groupBy(["ads_device_reports.report_date"]);
    ...
    public function index()
    {
       ...
       // $tdata['button_id']="user_modal";
        ...
        $tdata['display_contents'] = [
            ...
            "last_active"=>"Report date",
            "action"=>"Action"
            /*"action"=>"Action"*/
            ];
        /*$data['ajax_url']=base_url("Masters/Device/save_details");*/
        $tdata ['ajax_url_for_data']=base_url("Customer/Device/table_ajax");
        $data['table'] = view("Layout/table-view",$tdata);
        // $data['table'] ="";
            $db =db_connect();
    ...
    public function table_ajax(){
        $postData['ads_customer_device.user_id'] =$this->user_id;
            if($this->customer_id!=0)
                $postData['customer_id'] = $this->customer_id;
                // $sql = "*,(select concat(first_name,' ',last_name,' ',' ( ',company_name,' ) ') from ads_user where ads_user.user_id in ( select ads_customer_device.user_id from ads_customer_device where ads_customer_device.device_id=ads_device_master.device_id and status=1 ) limit 1) as company_name from ads_device_master,"
       //*,
        ...
        $tdata['table_data'] = $this->BaseModel->
        select("*,(select max(update_date) from ads_device_reports where device_id = ads_device_master.device_uuid) last_active,(select concat(first_name,' ',last_name,' ',' ( ',company_name,' ) ') from ads_user where ads_user.user_id in ( select ads_customer_device.user_id from ads_customer_device where ads_customer_device.device_id=ads_device_master.device_id and status=1 ) limit 1) as company_name ")
        ->join("ads_customer_device","ads_customer_device.device_id=ads_device_master.device_id and ads_customer_device.status=1 ","inner")
        ->where($postData)->findAll();
        foreach($tdata['table_data'] as $key=>$row){
            
            $tdata['table_data'][$key]['sr_no']=$key+1;
            $active = '<button class="btn btn-icon btn-success" onclick="window.location.href=\''.base_url("Customer/Device/report/".$row['device_uuid']).'\'">
											<span class="indicator-label svg-icon svg-icon-2">
...
// 			$tdata['table_data'][$key]['action']=$edit.$active.$deactive.$delete;
			$tdata['table_data'][$key]['action']=$active;
