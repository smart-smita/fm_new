...
protected $BaseModel;
   ...
   public function __construct(){
        helper("form");
     $db = null;
     $db['table']         = 'alert_nature_incident';
     $db['allowedFields'] = ['incident_name','status'];
     $db['primaryKey'] = "incident_id";
    ...
    public function index()
    {
        ...
        $tdata['title']="Incident";
        $tdata['button_name']="Add Incident";
        $tdata['button_id']="user_modal";
        
        $tdata['display_contents'] = [
            "incident_id"=>"ID",
            "incident_name"=>"User Name",
            "action"=>"Action"
            ];
        $data['ajax_url']=base_url("Masters/Incident/save_details");
        $tdata ['ajax_url_for_data']=base_url("Masters/Incident/table_ajax");
        $data['user_designation'] = ["Account Manager","Reporting manager",""];
        $data['table'] = view("Layout/table-view",$tdata);
        // $data['table'] ="";
        return view("Master/add_incident",$data);
    }
    public function table_ajax(){
        // if($_SESSION['role']!="super_admin")
        //     $tdata['table_data'] = $this->BaseModel->where(["employee_reporting_to"=>$_SESSION['incident_id']])->findAll();
        // else
        ...
        foreach($tdata['table_data'] as $key=>$row){
            $active = '<button class="btn btn-icon btn-success" onclick="url_call_ajax(\''.base_url("Masters/Incident/save_details/".$row['incident_id']).'/active\',$(this));">
			...
			$deactive = '<button  class="btn btn-icon btn-danger" onclick="url_call_ajax(\''.base_url("Masters/Incident/save_details/".$row['incident_id']).'/deactive\',$(this));">
                			<span class="indicator-label svg-icon svg-icon-2">
				...
				$delete = '<button data-ajax-url="'.base_url("Masters/Incident/save_details/".$row['incident_id']).'/delete" class="btn btn-icon btn-danger" onclick="delete_row(this);">
                			<span class="indicator-label svg-icon svg-icon-2">
            ...
            //$(this).attr('data-kt-indicator', 'on');$(this).attr('disabled', true);setTimeout(function (obj) {obj.attr('data-kt-indicator', 'off');obj.attr('disabled', false);},500,$(this));
             $edit = '<button data-ajax-url="'.base_url("Masters/Incident/get_form_data/".$row['incident_id']).'" class="btn btn-icon btn-primary" onclick="edit_id(this,'.$row['incident_id'].');">
