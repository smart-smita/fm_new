...
class Cluster_master extends BaseController
{
    /**
 * @var CRUDBaseModel
 */
protected $BaseModel;
   ...
   public function __construct(){
        helper("form");
     $db = null;
     $db['table']         = 'alert_cluster_master';
     $db['allowedFields'] = [
       'cluster_name','status'];
     $db['primaryKey'] = "cluster_id";
    ...
    public function index()
    {
        ...
        $tdata['title']="Cluster";
        $tdata['button_name']="Add Cluster";
        $tdata['button_id']="cluster_modal";
        ...
        $data['ajax_url']=base_url("Masters/Cluster_master/save_details");
        $tdata ['ajax_url_for_data']=base_url("Masters/Cluster_master/table_ajax");
        $data['table'] = view("Layout/table-view",$tdata);
        
        return view("Master/add_cluster_master",$data);
    }
    public function table_ajax(){
        ...
        foreach($tdata['table_data'] as $key=>$row){
            ...
            $active = '<button class="btn btn-icon btn-success" onclick="url_call_ajax(\''.base_url("Masters/Cluster_master/save_details/".$row['cluster_id']).'/active\',$(this));">
			...
			$deactive = '<button  class="btn btn-icon btn-danger" onclick="url_call_ajax(\''.base_url("Masters/Cluster_master/save_details/".$row['cluster_id']).'/deactive\',$(this));">
                			<span class="indicator-label svg-icon svg-icon-2">
				...
				$delete = '<button data-ajax-url="'.base_url("Masters/Cluster_master/save_details/".$row['cluster_id']).'/delete" class="btn btn-icon btn-danger" onclick="delete_row(this);">
                			<span class="indicator-label svg-icon svg-icon-2">
            ...
            //$(this).attr('data-kt-indicator', 'on');$(this).attr('disabled', true);setTimeout(function (obj) {obj.attr('data-kt-indicator', 'off');obj.attr('disabled', false);},500,$(this));
             $edit = '<button data-ajax-url="'.base_url("Masters/Cluster_master/get_form_data/".$row['cluster_id']).'" class="btn btn-icon btn-primary" onclick="edit_id(this,'.$row['cluster_id'].');">
