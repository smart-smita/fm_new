...
class Lmra_category extends BaseController
{
    /**
 * @var CRUDBaseModel
 */
protected $BaseModel;
   ...
   public function __construct(){
        helper("form");
     $db = null;
     $db['table']         = 'alert_lmra_categorys';
     $db['allowedFields'] = ['category_name','status'];
    ...
    public function index()
    {
        ...
        $data['ajax_url']=base_url("Masters/Lmra_category/save_details");
        $tdata ['ajax_url_for_data']=base_url("Masters/Lmra_category/table_ajax");
        $data['user_designation'] = ["Account Manager","Reporting manager",""];
        $data['table'] = view("Layout/table-view",$tdata);
        // $data['table'] ="";
        return view("Master/add_lmra_category",$data);
    }
    public function table_ajax(){
        ...
        foreach($tdata['table_data'] as $key=>$row){
            $active = '<button class="btn btn-icon btn-success" onclick="url_call_ajax(\''.base_url("Masters/Lmra_category/save_details/".$row['category_id']).'/active\',$(this));">
			...
			$deactive = '<button  class="btn btn-icon btn-danger" onclick="url_call_ajax(\''.base_url("Masters/Lmra_category/save_details/".$row['category_id']).'/deactive\',$(this));">
                			<span class="indicator-label svg-icon svg-icon-2">
				...
				$delete = '<button data-ajax-url="'.base_url("Masters/Lmra_category/save_details/".$row['category_id']).'/delete" class="btn btn-icon btn-danger" onclick="delete_row(this);">
                			<span class="indicator-label svg-icon svg-icon-2">
            ...
            //$(this).attr('data-kt-indicator', 'on');$(this).attr('disabled', true);setTimeout(function (obj) {obj.attr('data-kt-indicator', 'off');obj.attr('disabled', false);},500,$(this));
             $edit = '<button data-ajax-url="'.base_url("Masters/Lmra_category/get_form_data/".$row['category_id']).'" class="btn btn-icon btn-primary" onclick="edit_id(this,'.$row['category_id'].');">
