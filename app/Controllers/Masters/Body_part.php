...
class Body_part extends BaseController
{
...
protected $BaseModel;
   ...
   public function __construct(){
        helper("form");
     $db = null;
     $db['table']         = 'alert_body_part';
     $db['allowedFields'] = ['body_part_name','status'];
     $db['primaryKey'] = "body_part_id";
    ...
    public function index()
    {
        ...
        $tdata['title']="Body Part";
        $tdata['button_name']="Add Body Part";
        $tdata['button_id']="user_modal";
        
        $tdata['display_contents'] = [
            "body_part_id"=>"ID",
            "body_part_name"=>"User Name",
            "action"=>"Action"
            ];
        $data['ajax_url']=base_url("Masters/Body_part/save_details");
        $tdata ['ajax_url_for_data']=base_url("Masters/Body_part/table_ajax");
        $data['user_designation'] = ["Engineer","Reporting manager",""];
        $data['table'] = view("Layout/table-view",$tdata);
        // $data['table'] ="";
        return view("Master/add_body_part",$data);
    }
    public function table_ajax(){
        // if($_SESSION['role']!="super_admin")
        //     $tdata['table_data'] = $this->BaseModel->where(["employee_reporting_to"=>$_SESSION['body_part_id']])->findAll();
        // else
        ...
        foreach($tdata['table_data'] as $key=>$row){
            $active = '<button class="btn btn-icon btn-success" onclick="url_call_ajax(\''.base_url("Masters/Body_part/save_details/".$row['body_part_id']).'/active\',$(this));">
			...
			$deactive = '<button  class="btn btn-icon btn-danger" onclick="url_call_ajax(\''.base_url("Masters/Body_part/save_details/".$row['body_part_id']).'/deactive\',$(this));">
                			<span class="indicator-label svg-icon svg-icon-2">
				...
				$delete = '<button data-ajax-url="'.base_url("Masters/Body_part/save_details/".$row['body_part_id']).'/delete" class="btn btn-icon btn-danger" onclick="delete_row(this);">
                			<span class="indicator-label svg-icon svg-icon-2">
            ...
            //$(this).attr('data-kt-indicator', 'on');$(this).attr('disabled', true);setTimeout(function (obj) {obj.attr('data-kt-indicator', 'off');obj.attr('disabled', false);},500,$(this));
             $edit = '<button data-ajax-url="'.base_url("Masters/Body_part/get_form_data/".$row['body_part_id']).'" class="btn btn-icon btn-primary" onclick="edit_id(this,'.$row['body_part_id'].');">
