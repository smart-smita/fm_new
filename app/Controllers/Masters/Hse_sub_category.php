<?php

namespace App\Controllers\Masters;
use App\Controllers\BaseController;
use App\Models\CRUDBaseModel;

class Hse_sub_category extends BaseController
{
    /**
     * @var CRUDBaseModel
     */
    protected $BaseModel;

    public function __construct(){
        helper("form");
        $db = null;
        $db['table']         = 'alert_hse_sub_category';
        $db['allowedFields'] = ['site_category_id','sub_category_name','status'];
        $db['primaryKey']    = "sub_category_id";
        $this->BaseModel = new CRUDBaseModel($db);
    }

    public function index()
    {
        $data = [];

        $tdata['title']       = "HSE Sub Category";
        $tdata['button_name'] = "Add Sub Category";
        $tdata['button_id']   = "SubCategory";

        $tdata['display_contents'] = [
            "sub_category_id"    => "ID",
            "category_name"      => "Site Category",
            "sub_category_name"  => "Sub Category",
            "action"             => "Action"
        ];

        $data['ajax_url'] = base_url("Masters/Hse_sub_category/save_details");
        $tdata['ajax_url_for_data'] = base_url("Masters/Hse_sub_category/table_ajax");
        $data['table'] = view("Layout/table-view", $tdata);

        // Load Site Categories for the dropdown
        $db = db_connect();
        $data['site_category_list'] = $db->table("alert_hse_site_category")
                                         ->where("status", "1")
                                         ->get()
                                         ->getResultArray();

        return view("Master/add_hse_sub_category", $data);
    }

    public function table_ajax()
    {
        $db = db_connect();
        // Join with site category to show category name
        $tdata['table_data'] = $db->table('alert_hse_sub_category')
            ->select('alert_hse_sub_category.*, alert_hse_site_category.site_category_name')
            ->join('alert_hse_site_category', 'alert_hse_site_category.site_category_id = alert_hse_sub_category.site_category_id', 'left')
            ->get()
            ->getResultArray();

        $statusMessages = [
            0 => '<span class="badge badge-warning">Pending</span>',
            1 => '<span class="badge badge-info">Verified</span>',
            2 => '<span class="badge badge-success">Active</span>',
            3 => '<span class="badge badge-secondary">Deactivated</span>'
        ];

        foreach($tdata['table_data'] as $key => $row){
            $tdata['table_data'][$key]['sr_no'] = $key + 1;
            $tdata['table_data'][$key]['category_name'] = $row['site_category_name'] ?: '-';

            $active = '<button class="btn btn-icon btn-success" onclick="url_call_ajax(\''.base_url("Masters/Hse_sub_category/save_details/".$row['sub_category_id']).'/active\',$(this));">
                <span class="indicator-label svg-icon svg-icon-2"><i class="fa fa-unlock"></i></span>
                <span class="indicator-progress"><span class="spinner-border spinner-border-sm align-middle ms-2"></span></span>
            </button> ';

            $deactive = '<button class="btn btn-icon btn-danger" onclick="url_call_ajax(\''.base_url("Masters/Hse_sub_category/save_details/".$row['sub_category_id']).'/deactive\',$(this));">
                <span class="indicator-label svg-icon svg-icon-2"><i class="fa fa-lock"></i></span>
                <span class="indicator-progress"><span class="spinner-border spinner-border-sm align-middle ms-2"></span></span>
            </button> ';

            $delete = '<button data-ajax-url="'.base_url("Masters/Hse_sub_category/save_details/".$row['sub_category_id']).'/delete" class="btn btn-icon btn-danger" onclick="delete_row(this);">
                <span class="indicator-label svg-icon svg-icon-2"><i class="fa fa-trash"></i></span>
                <span class="indicator-progress"><span class="spinner-border spinner-border-sm align-middle ms-2"></span></span>
            </button> ';

            $edit = '<button data-ajax-url="'.base_url("Masters/Hse_sub_category/get_form_data/".$row['sub_category_id']).'" class="btn btn-icon btn-primary" onclick="edit_id(this,'.$row['sub_category_id'].');">
                <span class="indicator-label svg-icon svg-icon-3"><i class="fa fa-edit"></i></span>
                <span class="indicator-progress"><span class="spinner-border spinner-border-sm align-middle ms-2"></span></span>
            </button>';

            if($row['status'] == "0"){
                $deactive = "";
                $tdata['table_data'][$key]['tr_class'] = "bg-light-warning";
            } else if($row['status'] == "1"){
                $active = "";
            } else if($row['status'] == "2"){
                $delete = "";
                $deactive = "";
                $tdata['table_data'][$key]['tr_class'] = "bg-light-danger";
            }

            $tdata['table_data'][$key]['action'] = "<center>".$statusMessages[$row['status']]."<br><br>".$active.$deactive.$delete.$edit."</center>";
        }

        $tdata['data'] = $tdata['table_data'];
        unset($tdata['table_data']);
        return $this->response->setJSON($tdata);
    }

    public function get_form_data($id)
    {
        $responce['status']  = "0";
        $responce['message'] = "Details not found";
        if(isset($id)){
            $responce['data']    = $this->BaseModel->find($id);
            $responce['status']  = "1";
            $responce['message'] = "Details found";
        }
        echo json_encode($responce);
    }

    /**
     * AJAX: Get sub-categories by site_category_id
     * Used for dependent dropdown in HSE Client form
     */
    public function get_by_category()
    {
        $request = service('request');
        $site_category_id = $request->getVar('site_category_id');

        $db = db_connect();
        $data = $db->table('alert_hse_sub_category')
                   ->where('site_category_id', $site_category_id)
                   ->where('status', '1')
                   ->get()
                   ->getResultArray();

        return $this->response->setJSON([
            'status' => 1,
            'data'   => $data
        ]);
    }

    public function save_details($id = null, $action = null)
    {
        $request  = service('request');
        $postData = $request->getVar();

        if(isset($postData['honeypot'])){
            unset($postData['honeypot']);
        }

        if(isset($id)){
            $responce['message'] = "Data updation failed";
            if(isset($action)){
                switch($action){
                    case "active":   $postData['status'] = "1"; break;
                    case "deactive": $postData['status'] = "0"; break;
                    case "delete":   $postData['status'] = "2"; break;
                }
            }
            if($this->BaseModel->update($id, $postData)){
                $responce['status']  = "1";
                $responce['message'] = "Data saved successfully";
            }
        } else {
            $postData['status'] = "1";
            $responce['status']  = "0";
            $responce['message'] = "Data insertion failed";
            if($this->BaseModel->insert($postData)){
                $responce['status']  = "1";
                $responce['message'] = "Data saved successfully";
            }
        }
        echo json_encode($responce);
    }
}
