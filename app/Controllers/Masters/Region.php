...
protected $BaseModel;
   ...
   public function __construct(){
    ...
    public function index()
    {
        ...
        $tdata['title']="Region";
        $tdata['button_name']="Add Region";
        $tdata['button_id']="Region";
        ...
        $data['ajax_url']=base_url("Masters/Region/save_details");
        $tdata ['ajax_url_for_data']=base_url("Masters/Region/table_ajax");
        $data['table'] = view("Layout/table-view",$tdata);
        
        return view("Master/add_region",$data);
    }
    public function table_ajax(){
        ...
        foreach($tdata['table_data'] as $key=>$row){
            ...
            $active = '<button class="btn btn-icon btn-success" onclick="url_call_ajax(\''.base_url("Masters/Region/save_details/".$row['region_id']).'/active\',$(this));">
			...
			$deactive = '<button  class="btn btn-icon btn-danger" onclick="url_call_ajax(\''.base_url("Masters/Region/save_details/".$row['region_id']).'/deactive\',$(this));">
                			<span class="indicator-label svg-icon svg-icon-2">
				...
				$delete = '<button data-ajax-url="'.base_url("Masters/Region/save_details/".$row['region_id']).'/delete" class="btn btn-icon btn-danger" onclick="delete_row(this);">
                			<span class="indicator-label svg-icon svg-icon-2">
            ...
            //$(this).attr('data-kt-indicator', 'on');$(this).attr('disabled', true);setTimeout(function (obj) {obj.attr('data-kt-indicator', 'off');obj.attr('disabled', false);},500,$(this));
             $edit = '<button data-ajax-url="'.base_url("Masters/Region/get_form_data/".$row['region_id']).'" class="btn btn-icon btn-primary" onclick="edit_id(this,'.$row['region_id'].');">
