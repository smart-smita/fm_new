...
class Fm_nc_tracker extends BaseController
{
    /**
     * @var CRUDBaseModel
     */
  ...
  public function __construct(){
    ...
    public function index()
    {
        ...
        $tdata['title']="";
        // $tdata['button_name']="Add Fm_nc_tracker";
        ...
        $data['ajax_url']=base_url("Masters/Fm_nc_tracker/save_details");
        $tdata ['ajax_url_for_data']=base_url("Masters/Fm_nc_tracker/table_ajax").($qs ? ('?'.$qs) : '');
        // $data['user_designation'] = ["Engineer","Reporting manager",""];
        
         $datatop='<div class="row g-5 g-xl-8">
    <div class="col-md-4">
        <a href="'.base_url('Masters/Fm_nc_tracker/template_type_filter/0').'" class="card bg-primary hoverable mb-xl-8">
            <!--begin::Body-->
            <div class="card-body" style="padding: 1rem 2.25rem;">
               
                <div class="fw-semibold text-gray-100">Open</div>
            </div>
        </a>
    </div>
    
    <div class="col-md-4">
        <a href="'.base_url('Masters/Fm_nc_tracker/template_type_filter/2').'" class="card bg-dark hoverable mb-xl-8">
            <div class="card-body" style="padding: 1rem 2.25rem;">
                <div class="fw-semibold text-gray-100">Close</div>
            </div>
        </a>
    </div>
    
    <div class="col-md-4">
        <a href="'.base_url('Masters/Fm_nc_tracker/template_type_filter/1').'" class="card bg-warning hoverable mb-xl-8">
            <div class="card-body" style="padding: 1rem 2.25rem;">
                <div class="fw-semibold text-white">Working</div>
            </div>
        </a>
    </div>
...
';
        ...
        $db2 = db_connect();
        $regions  = $db2->table('alert_region')->select('region_name')->where('status !=', 2)->get()->getResultArray();
        ...
        $clusterBuilder = $db2->table('alert_location_master')->select('DISTINCT(cluster_name) cluster_name', false)->where('status !=', 2);
        if (!empty($userClusterACL)) {
            $clusterBuilder->whereIn('cluster_name', $userClusterACL);
        }
        $clusters = $clusterBuilder->get()->getResultArray();

        $locBuilder = $db2->table('alert_location_master')->select('location_name')->where('status !=', 2);
        if (!empty($userClusterACL)) {
        ...
        return view("Master/add_fm_nc_tracker",$data);
    }
    public function template_type_filter($nc_status)
    {
        $data = [];

        $tdata['title']="Fm Nc Tracker";
        // $tdata['button_name']="Add Fm_nc_tracker";
        $tdata['button_id']="user_modal";
        // changes on 10/11/12 by darsh: pass multi-filter qs to ajax url
        $req = service('request');
        ...
        $data['ajax_url']=base_url("Masters/Fm_nc_tracker/save_details");
        $tdata ['ajax_url_for_data']=base_url("Masters/Fm_nc_tracker/table_ajax/".$nc_status).($qs ? ('?'.$qs) : '');
        // $data['user_designation'] = ["Engineer","Reporting manager",""];
        
        // NEW CHANGES: Add dynamic counters for current filter (fm_leased = 'NO')
        $db2 = db_connect();
        $countQuery = $db2->query("SELECT COUNT(*) AS found_count, nc_status FROM alert_hse_audit_details WHERE fm_leased = 'NO' GROUP BY nc_status");
  ...
  public function table_ajax($nc_status=null)
{
    ...
    $sql = "SELECT 
                ...
                alert_hse_audit_details.fm_leased, 
            ...
            WHERE alert_hse_audit_details.fm_leased = 'NO'";
    ...
    if (isClusterManager() || isWHManager()) {
        ...
        if (!empty($userClients)) {
            ...
            $sql .= " AND alert_hse_audit_master.client_name IN ($placeholders)";
    ...
    $query = $db->query($sql, $params)->getResultArray();
     // changes on 10/11/12 by darsh: align labels with OE mapping
     $statusMessages = [                
    ...
    foreach ($query as $row) {
        $active = '<button class="btn btn-icon btn-success" title="Move to Working" onclick="updateFmStatus(' . $row['id'] . ', \'working\');">
        ...
        $edit = '<button data-ajax-url="'.base_url("Masters/Fm_nc_tracker/get_form_data/".$row['id']).'" class="btn btn-icon btn-primary" title="Edit / Submit" onclick="edit_id(this,'.$row['id'].');">
        ...
        if($row['nc_status']=="0") {
            ...
            if (isClusterManager() || isWHManager()) {
                ...
                $active = '<button class="btn btn-icon btn-success me-2" title="Approve & Close" onclick="updateFmStatus(' . $row['id'] . ', \'closed\');">
                        <span class="indicator-label svg-icon svg-icon-2">
                            <i class="fa fa-check"></i>
                        </span>
                    </button>';
                $edit = '<button class="btn btn-icon btn-danger" title="Send Back to Working" onclick="updateFmStatus(' . $row['id'] . ', \'working\');">
                        <span class="indicator-label svg-icon svg-icon-2">
        ...
        //$attachment_path = !empty($row['attachment']) ? base_url($row['attachment']) : '';
   ...
   public function get_form_data($id) {
    ...
    // exit();
                if(isset($postData['honeypot']))
                    ...
                    if (!isset($postData['fm_leased']) || empty($postData['fm_leased'])) {
                    // NEW CHANGES: Keep records visible in this tracker; default fm_leased to 'NO'
                    $postData['fm_leased'] = "NO";
                }
                ...
                if(isset($postData['before_photo_url'])) unset($postData['before_photo_url']);
                         ...
                         if( $_FILES['nc_after_photo']['error'] == UPLOAD_ERR_OK){
                             ...
                             if (in_array($ext, $allowed)) {
                               $folder="uploads/nc_after_photo/";
                                ...
                                // NEW CHANGES: Auto set Closed when after photo uploaded
                     ...
                     if(isset($postData['after_photo_url']))
                             ...
                             $allowed = array('jpg', 'jpeg', 'png', 'gif', 'webp');
                             if(isset($_FILES['attachment']) &&  isset( $_FILES['attachment']['error'])){
                        if( $_FILES['attachment']['error'] == UPLOAD_ERR_OK){
                            $ext = pathinfo($_FILES["attachment"]["name"], PATHINFO_EXTENSION);
                            if (in_array($ext, $allowed)) {
                              $folder = WRITEPATH."uploads/audit_files/AUD122/";
                                $url = "";
                                if(isset($postData['attachment_url']))
                                $url = $this->uploadImage($folder,"attachment",$postData['attachment_url']);
                                else
                                $url = $this->uploadImage($folder,"attachment");
                        ...
                        if(isset($postData['attachment_url']))
                                $postData['attachment_url'] = $postData['attachment_url'];
                        
                    }
                    if(isset($postData['attachment_url']))
                            unset($postData['attachment_url']);
    ...
    public function update_nc_status() {
        ...
        if ($id && $status) {
            ...
            // ACL: Check if Cluster Manager is trying to approve/reject or modify closed NC - 12/11/25
            if (isClusterManager() || isWHManager()) {
                $currentRecord = $db->table('alert_hse_audit_details')
                    ->where('id', $id)
                    ->get()
                ...
                if ($currentRecord && isset($currentRecord['nc_status'])) {
                    ...
                    // Block approve/reject actions (status 2 → 3)
                    if ($currentStatus == 2 && ($status === 'closed' || $status === 'reviewed')) {
                        $response['status'] = "0";
                        $response['message'] = "Cluster Managers cannot approve NCs. Only Auditors can approve.";
                        return $this->response->setJSON($response);
                    }
                    
                    // Block any modification to closed NCs (status 3)
                    if ($currentStatus == 3) {
                        $response['status'] = "0";
                        $response['message'] = "Cluster Managers cannot modify closed NCs";
                        return $this->response->setJSON($response);
                    }
                }
            }
