...
<!--begin::Item-->
    <li class="breadcrumb-item text-muted">
        ...
        <a href="<?= base_url('Masters/Oe_nc_tracker') ?>" class="text-muted text-hover-primary"><?=isset($title)?$title:"Client"?></a>
    </li>
<!--end::Item-->
<?php $this->endSection();?>    
...
function downloadMonthlyReport(format) {
    ...
    // URL pattern: Masters/Oe_nc_tracker/index/{status}?filters...
    ...
    let url = '';
    if (format === 'pdf') {
        url = '<?= base_url('Masters/Oe_nc_tracker/monthly_report_pdf') ?>' + (params.toString() ? '?' + params.toString() : '');
    } else if (format === 'excel') {
        url = '<?= base_url('Masters/Oe_nc_tracker/export_report_excel') ?>' + (params.toString() ? '?' + params.toString() : '');
    }
                    ...
                    <div class="scroll-y me-n7 pe-7" id="user" data-kt-scroll="false"
                        ...
                        <div class="row mb-12">
                            ...
                            <div class="twocol">
                                ...
                                <div class="col-md-12 fv-row fv-plugins-icon-container">
                                    <label class="fs-5 fw-bold mb-2">Audit Parameter</label>
                                    <input type="text" class="form-control form-control-solid" id="audit_parameter" name="audit_parameter" disabled>
                                </div>
    ...
    ajax_call(url,formData,$(obj),function(responce){
        try{
            ...
            if(responce.status==1){
                ...
                $("#<?=$form_id?>").find("#audit_parameter").val(d.audit_parameter).prop("disabled", true);
...
function updateOeStatus(btn, id, status) {
    ...
    $.ajax({
        url: '<?= base_url('Masters/Oe_nc_tracker/update_status') ?>',
        method: 'POST',
        dataType: 'json',
...
<!-- hide Add button on OE NC Tracker page only -->
<script>
$(function(){
...
function submitBulkClose() {
    ...
    $.ajax({
        url: '<?= base_url('Masters/Oe_nc_tracker/bulk_close_nc') ?>',
        type: 'POST',
        dataType: 'json',
        data: {
