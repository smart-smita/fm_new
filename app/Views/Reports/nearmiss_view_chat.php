...
<div class="col-md-12 mb-5">
							<div class="card">
								<div class="card-header" style="display: flex; justify-content: space-between; align-items: center; padding: 10px;">
                                    <span><h3>Brif Details Of <?=$details['near_miss_category_text']?> </h3></span>
                                    <button class="btn btn-info" style="float: right;" onclick="event.preventDefault(); $('#details_div').toggle('show');">Show/Hide</button>
                                </div>
            ...
            <div class="card-user-profile">
                <div class="profile-page-left">
                   ...
                   <hr>
                                            ...
                                            <h3> 
                                        <?php 
                                        // changes on 9/10/25 by darsh: show each member with a remove (x) button
                                        $involvedRaw = isset($details['involved_user']) ? (string)$details['involved_user'] : '';
                                        $names = array_filter(array_map('trim', $involvedRaw !== '' ? explode(',', $involvedRaw) : []));
                                        $idsRaw = isset($details['group_member_ids']) ? (string)$details['group_member_ids'] : '';
                                        $ids = array_filter(array_map('trim', $idsRaw !== '' ? explode(',', $idsRaw) : []));
                                        foreach ($names as $idx => $name){
                                            $uid = isset($ids[$idx]) ? (int)$ids[$idx] : null;
                                            echo "<span class='badge badge-info me-2'>".htmlspecialchars($name, ENT_QUOTES)." ";
                                            if ($uid){
                                                $removeUrl = base_url(index_page().'/Reports/Near_miss_reports/remove_engineer?near_miss_id='.$details['near_miss_id'].'&user_id='.$uid);
                                                echo "<a href='".$removeUrl."' class='text-white ms-2' title='Remove' onclick=\"return confirm('Remove this member?');\">&times;</a>";
                                            }
                                            echo "</span> ";
                                        }
                                        ?>
										...
										<hr>
                    <div>
                        <?php // changes on 9/10/25 by darsh: fix action to correct controller method path ?>
                        <form action="<?=base_url(index_page().'/Reports/Near_miss_reports/add_engineer')?>" method="post" id="user_list_involved" class="login-form">
                            <div class="row">
                                <div class="col-md-12">
                    ...
                    <form action="<?=$form_action?>" method="post">
                        <div class="comment-block">
                            <div class="form-group">
                                <textarea name="comment_text" class="form-control" id="comment-textarea" rows="2" placeholder="Enter your comment here..." required></textarea>
                        ...
                        <?php // changes on 9/10/25 by darsh: post only near_miss_id; user_id taken server-side from session ?>
                        <input type="hidden" value="<?=$details['near_miss_id']?>" name="near_miss_id">
                    </form>

                    <hr>
                    <?php // changes on 9/10/25 by darsh: match LMRA comment list styling ?>
                    <div class="mt-5">
                        <?php foreach($near_miss_chat as $rows) { ?>
                        <div class="d-flex align-items-start mb-4">
                            <div class="me-3">
                                <img src="<?=base_url('assets/images/profile.png')?>" width="44" height="44" class="rounded-circle">
                            </div>
                            <div class="flex-grow-1">
                                ...
                                <div>
                                    <?php if (isset($rows['chat_type']) && (int)$rows['chat_type'] === 1) { ?>
                                        <a href="<?=$rows['chat_image']?>">
                                            <img src="<?=$rows['chat_image']?>" class="img-fluid img-thumbnail" style="max-width: 240px;">
                                        </a>
                                    <?php } else { ?>
                                        <p class="mb-0" style="white-space:pre-wrap; word-break:break-word;"><?=$rows['chat_text']?></p>
                                    <?php } ?>
                                </div>
                            </div>
                        </div>
