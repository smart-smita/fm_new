...
?>
            ...
            <div class="card card-custom">
                <div class="card-header">
                    <div class="card-title">
                        <h3 class="card-label"><?= $audit_name ?> Score</h3>
                    </div>
                    <!-- changes on 7/10/25 by darsh: remove blue performed audits box as per request -->
                </div>
    ...
    <thead>
        <tr class="header-row">
            <th>Region</th>
            <?php
            // changes on 1/10/25 by darsh: robust month headers
            $months = array_values(array_unique(array_column($zone, 'audit_month')));
    ...
    <tbody>
        <?php
        // changes on 1/10/25 by darsh: group by region (fallback to zone)
        $zones = [];
        foreach ($zone as $row) {
            if (!empty($row['audit_month'])) {
                $zName = isset($row['region']) ? $row['region'] : (isset($row['zone']) ? $row['zone'] : 'Unknown');
                $zones[$zName][$row['audit_month']] = $row['yes'];
            }
        }

        // changes on 1/10/25 by darsh: display all regions present in data
        foreach ($zones as $zone_name => $months_data):
        ?>
            <tr>
                <td ><?= esc($zone_name); ?></td>
...
<div class="container mt-4">
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"><?=$audit_name?> Month Summary</h3>
                </div>
                <div class="card-body">
                  <table class="table table-bordered table-custom">
    <thead>
...
<div class="container mt-4">
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"><?=$audit_name?> West Summary</h3>
                </div>
                <div class="card-body">
...
<table class="table table-bordered table-custom">
    ...
    <tbody>
        ...
        <?php foreach ($west_score as $row): ?>
            <tr>
                <!-- changes on 1/10/25 by darsh: use region instead of zone -->
                <td><?= esc(isset($row['region']) ? $row['region'] : (isset($row['zone']) ? $row['zone'] : '')); ?></td>
...
<div class="container mt-4">
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"><?=$audit_name?> North Summary</h3>
                </div>
                <div class="card-body">
                    <!-- Table with proper borders -->
                   <table class="table table-bordered table-custom">
...
<div class="container mt-4">
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"><?=$audit_name?> South Summary</h3>
                </div>
                <div class="card-body">
...
<div class="container mt-4">
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"><?=$audit_name?> East Summary</h3>
                </div>
                <div class="card-body">
