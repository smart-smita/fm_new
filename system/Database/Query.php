...
class Query implements QueryInterface
{
    ...
    protected function matchNamedBinds(string $sql, array $binds): string
    {
        ...
        foreach ($binds as $placeholder => $value) {
            ...
            $escapedValue = $val1 ? $this->db->escape($val0) : $val0;
            ...
            if (is_array($val0)) {
                $escapedValue = '(' . implode(',', is_array($escapedValue) ? $escapedValue : [$escapedValue]) . ')';
            }
    ...
    protected function matchSimpleBinds(string $sql, array $binds, int $bindCount, int $ml): string
    {
        ...
        if ($bindCount === 0 || $c === 0 || empty($matches[0])) {
            return $sql;
        }

        $indexedBinds = array_values($binds);

        do {
            $c--;
            $item = $indexedBinds[$c] ?? null;
            if (! is_array($item)) {
                $item = [$item, true];
            }
            $val0 = $item[0] ?? null;
